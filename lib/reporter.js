...
class JsonReporter extends events.EventEmitter {
    ...
    onSuiteStart (event) {
        ...
        if (event.tags && event.tags.length > 0) {
            xrayId = event.tags[0].name.substring(1);
        } else {
        ...
        let results = Object.keys(browserToEnvMap)
            .map(browser => {
                ...
                if (this.options.testKey) {
                    result.info.summary = `Execution of test ${this.options.testKey} in browser: ${browser}`
                } else if (this.options.testSetKey) {
                    result.info.summary = `Execution of test set ${this.options.testSetKey} in browser: ${browser}`
                } else if (this.options.testPlanKey) {
                    result.info.summary = `Execution of test set ${this.options.testPlanKey} in browser: ${browser}`
                    result.info.testPlanKey = this.options.testPlanKey
                } else {
                    result.info.summary = `Execution of test undefined in browser: ${browser}`
                }
