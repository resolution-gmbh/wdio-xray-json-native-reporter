...
class JsonReporter extends events.EventEmitter {
    ...
    onSuiteStart (event) {
        ...
        if (event.tags && event.tags.length > 0 && event.parent !== null) {
            ...
            xrayId = event.tags.reduce((acc, tag) => {
                ...
                if (!acc && tag.line !== 1 && tag.name.substr(1, pId === this.options.testProject)) return tag.name.substring(1);
                else return acc;
            }, null);
        } else {
    ...
    onTestPass (event) {
        console.log(chalk.greenBright(`cucumber: Step passed: ${event.title}`));
    ...
    onTestFailOrPending (event) {
        event.title && console.log(chalk.redBright(`cucumber: Step failed: ${event.title}. Aborting test ...`));
        let suite = this.suitesUIDMap[event.parent];
        ...
        event.err.message += ` (Related Tests: ${JSON.stringify(event.tags.filter(tag => tag.name.includes(this.options.testProject)).map(tag => tag.name))})`;
        event.err.stack = event.err.stack && event.err.stack.substr(0, 2000) || null;
    ...
    write (json) {
        ...
        try {
            ...
            const filename = `WDIO.xray.json.${finishDate}_${Math.random().toString().substr(2,4)}.json`;
