...
class JsonReporter extends events.EventEmitter {
        ...
        if (event.tags && event.tags.length > 0 && event.parent !== null) {
            ...
            xrayId = event.tags.reduce((acc, tag) => {
                ...
                if (
                    !acc &&
                    tag.line !== 1 &&
                    tag.name.substr(1, pId === this.options.testProject)
                )
            ...
            xrayId = event.file
                .slice(event.file.lastIndexOf(path.sep) + 1)
                .replace(".feature", "");
        }
    ...
    onTestPass(event) {
        console.log(`cucumber: Step passed: ${event.title}`);
    ...
    onTestFailOrPending(event) {
        event.title &&
            console.log(
                `cucumber: Step failed: ${event.title}. Aborting test ...`
            );
        let suite = this.suitesUIDMap[event.parent];
        ...
        if (event.tags) {
            comment += JSON.stringify(event.tags.map((tag) => tag.name));
            comment += "\r\n";
        }
        if (event.err.stack) {
        ...
        // This is quite a hack and should be solved when webdriver.io is upgraded to a new major version
        // We induce some more information to the error message and shorten the stack trace.
        event.err.message += ` (Related Tests: ${JSON.stringify(
            event.tags
                .filter((tag) => tag.name.includes(this.options.testProject))
                .map((tag) => tag.name)
        )})`;
        event.err.stack =
            (event.err.stack && event.err.stack.substr(0, 2000)) || null;
    ...
    onEnd() {
        ...
        Object.keys(this.baseReporter.stats.runners).forEach((key) => {
            let browser = this.baseReporter.stats.runners[key]
        ...
        let results = Object.keys(browserToEnvMap).map((browser) => {
            let tests = this.suites.reduce((allTests, suite) => {
                let tests = suite.scenarios.reduce((result, current) => {
                    let steps = browserToEnvMap[browser].reduce(
                        (steps, env) => {
            ...
            if (this.options.additionalEnvironmentData) {
                if (Array.isArray(this.options.additionalEnvironmentData)) {
                    result.info.testEnvironments.push(
                        ...this.options.additionalEnvironmentData
                    );
                } else {
                    result.info.testEnvironments.push(
                        this.options.additionalEnvironmentData
            ...
            if (this.options.testKey) {
                result.info.summary = `Execution of test ${
                    this.options.testKey
                } in environment ${result.info.testEnvironments.join(", ")}`;
            } else if (this.options.testSetKey) {
                result.info.summary = `Execution of test set ${
                    this.options.testSetKey
                } in environment ${result.info.testEnvironments.join(", ")}`;
            } else if (this.options.testPlanKey) {
                result.info.summary = `Execution of test plan ${
                    this.options.testPlanKey
                } in environment ${result.info.testEnvironments.join(", ")}`;
                result.info.testPlanKey = this.options.testPlanKey;
            } else if (this.options.testExecutionKey) {
                result.info.summary = `Execution of test execution ${
                    this.options.testExecutionKey
                } in environment ${result.info.testEnvironments.join(", ")}`;
            } else {
    ...
    write(json) {
        ...
        const { finishDate } = json[0].info;

        try {
            const dir = path.resolve(this.options.outputDir);
            const filename = `WDIO.xray.json.${finishDate}_${Math.random()
                .toString()
                .substr(2, 4)}.json`;
            ...
            console.log(
                `Failed to write json report to [${this.options.outputDir}]. Error: ${e}`
            );
        }
    }
    ...
    upload(json) {
        json.forEach((entry) => {
            try {
                ...
                if (this.options.xrayUser && this.options.xrayPass) {
                    options.headers = {
                        authorization:
                            "Basic " +
                            new Buffer(
                                this.options.xrayUser +
                                    ":" +
                                    this.options.xrayPass,
                                "ascii"
                            ).toString("base64"),
                        "X-PUNCH-A-HOLE": "Yes",
                    };
                ...
                const res = request(
                    "POST",
                    `${this.options.xrayHost}/rest/raven/1.0/import/execution`,
                    options
                );
                const resBody = res.getBody("utf-8");
                if (res.statusCode >= 300) {
                    console.error(
                        `Error uploading results to XRay: ${resBody}`
                    );
                } else {
                    const body = JSON.parse(resBody);
                    console.log(
                        `Sucessfully uploaded test execution as ${body.testExecIssue.key}. You can access it via ${this.options.xrayHost}/browse/${body.testExecIssue.key}.`
                    );
                }
            } catch (err) {
